...
@magicgui(
    auto_call=True,
    layout='vertical',
    algorithm={'choice':['gaussian_mixture_model', 'gaussian_mixture_model']}
)
def __clustering_dock_widget(intensity : Image = None,
                                labels : Image = None,
                                algorithm : str = 'gaussian_mixture_model',
                                num_classes : int = 2,
                                train : bool = False,
    ...
    if intensity is None or labels is None:
        print("no data")
        return
    ...
    intensity = cle.push_zyx(intensity)
    labels = cle.push_zyx(labels)
    ...
    if (__clustering_dock_widget.initial_call):
        __clustering_dock_widget.self.viewer.add_image(prediction_map)
        __clustering_dock_widget.initial_call = False
    else:
        __clustering_dock_widget.self.layer.data = prediction_map
